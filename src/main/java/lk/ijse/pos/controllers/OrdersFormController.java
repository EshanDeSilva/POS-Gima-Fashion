...
public class OrdersFormController {
    ...
    private void setData(TreeItem<OrderTm> newValue) {
        ...
        try {
            txtQtyOnHand.setText(String.valueOf(ItemModel.find(txtItemCode.getText()).getQty()));
            txtSellingPrice.setText(String.valueOf(newValue.getValue().getUnitPrice()));
            txtProfit.setText(String.valueOf(newValue.getValue().getUnitPrice()-Double.parseDouble(ItemModel.find(txtItemCode.getText()).getBuyingPrice())));
    ...
    private void loadItem() {
        try {
            List<ItemDto> items = ItemModel.findAll();
    ...
    private void setItem() {
        if (cmbItemCode.getValue()!=null) {
            try {
                ...
                txtDescription.setText(ItemModel.find(txtItemCode.getText()).getDescription());
                txtQtyOnHand.setText(ItemModel.find(txtItemCode.getText()).getQty());
                txtSellingPrice.setText(String.format("%.2f",Double.parseDouble(ItemModel.find(txtItemCode.getText()).getSellingPrice())));
                txtProfit.setText(String.format("%.2f",Double.parseDouble(ItemModel.find(txtItemCode.getText()).getSellingPrice())-Double.parseDouble(ItemModel.find(txtItemCode.getText()).getBuyingPrice())));
                txtType.setText(ItemModel.find(txtItemCode.getText()).getCategoryDto().getType());
                txtSize.setText(ItemModel.find(txtItemCode.getText()).getCategoryDto().getSize());
    ...
    private void loadEmployers() {
        try {
            List<EmployerDto> employers = EmployerModel.findAll();
        ...
        cmbEmployerId.setOnAction(actionEvent -> {
            if (cmbEmployerId.getValue()!=null) {
                try {
                    txtEmployerName.setText(EmployerModel.find(cmbEmployerId.getValue().toString()).getName());
                } catch (SQLException | ClassNotFoundException e) {
                    e.printStackTrace();
                }
            }
                ...
                if (!txtEmployerName.getText().isEmpty()) {
                    ...
                    for (String name:nameList) {
                        if (name.toLowerCase().startsWith(txtEmployerName.getText().toLowerCase())){
                            ...
                            notSet=false;
                            try {
                                cmbEmployerId.setValue(EmployerModel.findByName(txtEmployerName.getText()).getId());
                    ...
                    try {
                        ...
                        for (OrderTm tm : cartList) {
                            dtoList.add(new OrderDetailsDto(
                                    ...
                                    tm.getQty(),
                                    tm.getUnitPrice(),
                                    (tm.getAmount()) - (Double.parseDouble(ItemModel.find(tm.getItemCode()).getBuyingPrice()) * tm.getQty()),
                                    tm.getDiscount()
                            ));
                        }
                        ...
                        Boolean isSaved = OrderModel.save(new OrderDto(
                                txtOrderId.getText(),
                                datePicker.getValue().toString(),
    ...
    private void updateStock() {
        for (OrderTm tm:cartList) {
            try {
                ItemModel.updateQty(new ItemDto(
                        tm.getItemCode(),
                        ItemModel.find(tm.getItemCode()).getSupplierId(),
                        tm.getDescription(),
                        String.valueOf(Integer.parseInt(ItemModel.find(tm.getItemCode()).getQty())-tm.getQty()),
                        String.valueOf(tm.getUnitPrice()),
                        ItemModel.find(tm.getItemCode()).getBuyingPrice(),
                        ItemModel.find(tm.getItemCode()).getCategoryDto()
                ));
            }catch (ClassNotFoundException | SQLException e) {
                e.printStackTrace();
            }
    ...
    private void print(){
        try {
            ...
            JasperExportManager.exportReportToPdfFile(jasperPrint,"E:/IJSE/GDSE66/1st_Semester Final Project/POS-Gima_Fashion/Bills/"+txtOrderId.getText()+".pdf");
    ...
    private void sendMessage(){
        ...
        String senderPassword = "qpdcerhawxzjrspl";
        ...
        try {
            ...
            String filename = "E:/IJSE/GDSE66/1st_Semester Final Project/POS-Gima_Fashion/Bills/"+txtOrderId.getText()+".pdf";//change accordingly
