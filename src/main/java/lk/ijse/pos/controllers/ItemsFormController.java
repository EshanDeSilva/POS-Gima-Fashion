...
import lk.ijse.pos.dto.tm.ItemTm;
...
public class ItemsFormController {
    ...
    private void loadTypes() {
        ...
        typeList.add("Custom");
        try {
            List<CategoryDto> categories = CategoryModel.findAllTypes();
    ...
    private void loadSizes() {
        ...
        sizeList.add("Custom");
        try {
            List<CategoryDto> categories = CategoryModel.findAllSize(cmbType.getValue().toString());
            for (CategoryDto dto:categories) {
    ...
    private void loadSuppliers() {
        try {
            List<SupplierDto> suppliers = SupplierModel.findAll();
        ...
        cmbId.setOnAction(actionEvent -> {
            if (cmbId.getValue()!=null) {
                try {
                    cmbSupplierName.setValue(SupplierModel.find(cmbId.getValue().toString()).getName());
                } catch (SQLException | ClassNotFoundException e) {
                    e.printStackTrace();
                }
            }
        ...
        cmbSupplierName.setOnAction(actionEvent -> {
            if (cmbSupplierName.getValue()!=null) {
                try {
                    cmbId.setValue(SupplierModel.findByName(cmbSupplierName.getValue().toString()).getSupplierId());
                } catch (SQLException | ClassNotFoundException e) {
                    e.printStackTrace();
                }
            }
    ...
    private void loadCode() {
        try {
            txtCode.setText(ItemModel.getCode());
            ...
            for (ItemDto dto:items) {
                ...
                btn.setOnAction(actionEvent -> {
                    ...
                    if (buttonType.get()==ButtonType.YES){
                        try {
                            Boolean isDeleted = ItemModel.delete(dto.getCode());
                            if (isDeleted) {
    ...
    public void saveButtonOnAction(ActionEvent actionEvent) {
        if (btnSave.getText().equals("Save") && !txtCode.getText().isEmpty() && cmbId.getValue()!=null &&
            ...
            //if (cmbSize.getValue()!=null && !cmbSize.getValue().toString().isEmpty() && !cmbSize.getValue().toString().equals("Custom")) {
                try {
                    Boolean isSaved = SupplierInvoiceModel.save(new SupplierInvoiceDto(SupplierInvoiceModel.getId(),
                            ...
                            CategoryModel.find(cmbType.getValue().toString(), cmbSize.getValue().toString()))
                    ));
                    if (isSaved) {
                        //loadCode();
                        clearDataFields();
            ...
            /*}else{
                ...
                cmbType.getValue()!=null && cmbSize.getValue()!=null && !cmbSize.getValue().equals("Custom")) {
            try {
                Boolean isUpdated = ItemModel.update(new ItemDto(
                        txtCode.getText(), cmbId.getValue().toString(), txtDescription.getText(),
                        txtQty.getText(), txtSellingPrice.getText(), txtBuyingPrice.getText(),
                        CategoryModel.find(cmbType.getValue().toString(), cmbSize.getValue().toString())
                ));
    ...
    public void addStockButtonOnAction(ActionEvent actionEvent) {
        if (btnSave.getText().equals("Update") && !txtCode.getText().isEmpty() && cmbId.getValue()!=null &&
            ...
            try {
                Boolean isUpdated = SupplierInvoiceModel.addStock(
                        new SupplierInvoiceDto(SupplierInvoiceModel.getId(),cmbId.getValue().toString(),
    ...
    public void addButtonOnAction(ActionEvent actionEvent) {
        ...
        try{
            Boolean isSaved = CategoryModel.save(new CategoryDto(CategoryModel.getId(), txtSize.getText(), type));
            if (isSaved){
