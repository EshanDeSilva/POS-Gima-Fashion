...
import lk.ijse.pos.dto.SalesReturnDetailsDto;
import lk.ijse.pos.dto.SalesReturnDto;
...
public class SalesReturnsFormController {
    ...
    private void getOrder() {
        txtOrderId.setOnAction(event -> {
            if (txtOrderId.getText().matches("^ORD-\\d{8,20}$")){
                ...
                double total=0;
                try {
                    list = OrderDetailsModel.getAll(txtOrderId.getText());
    ...
    public void placeReturnButtonOnAction(ActionEvent event) {
        try {
            String returnId = SalesReturnModel.getId();
            ...
            Boolean isSaved = SalesReturnModel.save(new SalesReturnDto(
                    returnId,
                    txtOrderId.getText(),
                    LocalDate.now().toString(),
            ...
            if (isSaved) {
                ...
                for (OrderTm tm:tmList) {
                    Boolean isAdded = ItemModel.addStock(tm.getItemCode(),tm.getQty());
                    if (!isAdded){
                        allAdded = false;
                    }
                }
