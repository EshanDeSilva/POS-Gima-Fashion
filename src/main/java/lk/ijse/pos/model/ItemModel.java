...
import lk.ijse.pos.dto.CategoryDto;
...
public class ItemModel {
    ...
    public static Boolean addStock(String code, int qty) throws SQLException, ClassNotFoundException {
        return CrudUtil.execute("UPDATE item SET qtyOnHand=? WHERE itemCode=?", String.valueOf(qty+Integer.parseInt(ItemModel.find(code).getQty())), code);
    }
    ...
    public static List<ItemDto> findAll() throws SQLException, ClassNotFoundException {
        ...
        while (resultSet.next()){
            list.add(new ItemDto(
                    ...
                    CategoryModel.find(resultSet.getString(7))
            ));
        }
        return list;
    }
